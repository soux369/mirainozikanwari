...
import { acquireVisionLock, releaseVisionLock } from './src/lib/visionLock';
...
function MainApp() {
    ...
    useEffect(() => {
        ...
        updateWidgets(current, settings, isPremium);
    }, [courses, currentTerm, settings, isPremium]);
    ...
    const updateCourse = (updated: Course) => {
        // Shared Data Logic (Assignments, Notes, Images)
        // If these changed, update all courses with same Code/Name in the same Term
        ...
        const notesChanged = original?.notes !== updated.notes;
        const imagesChanged = JSON.stringify(original?.images) !== JSON.stringify(updated.images);
        const attendanceChanged = JSON.stringify(original?.attendanceHistory) !== JSON.stringify(updated.attendanceHistory);

        if ((assignmentsChanged || notesChanged || imagesChanged || attendanceChanged) && original) {
            nextCourses = courses.map(c => {
                ...
                const isSameName = !updated.code && c.name === updated.name;
                ...
                if (isSameTerm && (isSameCode || isSameName)) {
                    return {
                        ...
                        attendanceHistory: updated.attendanceHistory
                    };
                }
                return c;
            });
    ...
    const processImageAttributes = async (uri: string) => {
        try {
            ...
            if (apiKey) {
                const runGemini = async () => {
                    ...
                    try {
                        ...
                        if (rawParsed.length > 0) {
                            ...
                            Alert.alert(t.error, t.detectionError);
                        }
                    } catch (aiErr: any) {
                        console.error(aiErr);
                        if (aiErr.message === 'VISION_BUSY') {
                            Alert.alert(
                                t.visionBusyTitle,
                                t.visionBusyMessage,
                                [
                                    { text: t.cancel, style: "cancel" },
                                    { text: t.tryAgain, onPress: () => runGemini() }
                                ]
                            );
                        } else {
                ...
                Alert.alert(t.confirm, t.startAiAnalysis, [
                    { text: t.cancel, style: "cancel" },
                    { text: t.ok, onPress: runGemini }
                ]);
            } else {
                if (!acquireVisionLock()) {
                    Alert.alert(
                        t.visionBusyTitle,
                        t.visionBusyLocalMessage,
                        [
                            { text: t.close, style: "cancel" },
                            { text: t.tryAgain, onPress: () => processImageAttributes(uri) }
                        ]
                    );
                    return;
                }
            ...
            Alert.alert(t.error, t.failedToLoadImage);
        }
    };
                    ...
                    { text: t.cancel, style: "cancel" },
                    {
                        ...
                        onPress: async () => {
                            try {
                                ...
                                if (status !== 'granted') {
                                    Alert.alert(t.permissionRestricted, t.cameraPermissionDenied);
                                    return;
                                }
    ...
    const handleConfirmFriendName = (name: string) => {
        ...
        Alert.alert(t.ok, t.friendImportSuccess(name));
    };
                            ...
                            try {
                                ...
                                if (data.type === 'complete') {
                                    ...
                                    if (parsed.length > 0) {
                                        ...
                                        Alert.alert("エラー", "授業を検出できませんでした。");
                                    }
                                } else if (data.type === 'error') {
                                    console.error("[OCR Error]", data.payload);
                                    Alert.alert("エラー", "解析中にエラーが発生しました: " + data.payload);
                                    setOcrTrigger(null);
                                    releaseVisionLock();
                                }
                            } catch (e) {
                        ...
                        <View style={styles.header}>
                            <View style={{ flex: 1, marginRight: 8 }}>
                                <MarqueeText
                                    ...
                                    style={{ color: isDarkMode ? '#94a3b8' : '#64748b', fontSize: 13, marginBottom: 4, fontWeight: '600' }}
                                />
                                <MarqueeText
                                    text={getGreeting()}
                                    style={{ color: textPrimary, fontSize: 24, fontWeight: '800', letterSpacing: -0.5 }}
                                />
                            ...
                            >
                                ...
                                <View style={{ width: SCREEN_WIDTH, flex: 1 }}>
                                    <SettingsScreen
                                        settings={settings}
                                        isDarkMode={isDarkMode}
                                        onClose={() => setActiveTab('timetable')}
                                        onSave={saveSettings}
