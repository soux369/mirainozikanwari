...
const VideoPlayerComponent = ({ uri }: { uri: string }) => {
...
const MediaThumbnail = ({ uri, onPress, onDeleteItem }: { uri: string, onPress: () => void, onDeleteItem: () => void }) => {
    const [thumb, setThumb] = useState<string | null>(null);

    React.useEffect(() => {
        if (isVideo(uri)) {
            VideoThumbnails.getThumbnailAsync(uri, { time: 1000 }).then(res => {
                setThumb(res.uri);
    ...
    return (
        <View style={{ marginRight: 12, position: 'relative' }}>
            <TouchableOpacity onPress={onPress}>
                <Image
                    source={{ uri: isVideo(uri) ? (thumb || uri) : uri }}
...
export const CourseDetailModal: React.FC<CourseDetailModalProps> = ({
    ...
    const removeAttendance = (status: AttendanceStatus) => {
        ...
        const statusLabel = status === 'present' ? '出席' : status === 'late' ? '遅刻' : status === 'absent' ? '欠席' : '取消';

        Alert.alert(
            "記録の削除",
            `最新の${statusLabel}記録を削除しますか？`,
            [
                { text: "キャンセル", style: "cancel" },
                {
                    text: "削除",
    ...
    const deleteAssignment = (id: string) => {
        ...
        Alert.alert(
            t.deleteAssignment,
            t.deleteAssignmentConfirm(assignment.title),
            [
    ...
    const handleAddPhoto = async () => {
        try {
            // Check for premium status and limit
            const currentImagesCount = (course.images || []).length;
            const FREE_LIMIT = 3;

            if (!isPremium && currentImagesCount >= FREE_LIMIT) {
                Alert.alert(
                    t.proPlanSection,
                    t.imageLimitReached(FREE_LIMIT),
                    [
            ...
            if (!result.canceled && result.assets && result.assets.length > 0) {
                ...
                const fileName = `course_media_${timestamp}.${ext}`;
        ...
        <View style={[StyleSheet.absoluteFill, { zIndex: 50, elevation: 50 }]}>
                                ...
                                <View style={styles.attendanceContainer}>
                                    ...
                                    <View style={styles.graphContainer}>
                                        <View style={{ width: 100, height: 100, justifyContent: 'center', alignItems: 'center' }}>
                                            ...
                                            <TouchableOpacity
                                                style={[styles.ringInner, { position: 'absolute' }]}
                                    ...
                                    >
                                        {course.images.map((img, idx) => (
                                            <MediaThumbnail
                                                ...
                                                onDeleteItem={() => handleDeleteImage(idx)}
                                            />
                                        ))}
                                    </ScrollView>
                                )}
                ...
                <View style={{ flex: 1, backgroundColor: 'black', justifyContent: 'center' }}>
                    ...
                    {course.images && course.images.length > 0 && (
                        <ScrollView
                            ...
                            pagingEnabled
                            showsHorizontalScrollIndicator={false}
                            contentOffset={{ x: (course.images.indexOf(viewingImage || '') || 0) * Dimensions.get('window').width, y: 0 }}
                        >
                            {course.images.map((img, idx) => (
                                <View key={idx} style={{ width: Dimensions.get('window').width, height: '100%', justifyContent: 'center', alignItems: 'center' }}>
                                    {isVideo(img) ? (
                                        <View style={{ width: '100%', height: '80%' }}>
                                            <VideoPlayerComponent uri={img} />
                                        </View>
                                    ) : (
                                        <Image
                                            source={{ uri: img }}
            ...
            >
                <View style={[styles.historyOverlay, { backgroundColor: 'rgba(0,0,0,0.5)' }]}>
                    <View style={[styles.historyContainer, { backgroundColor: bg }]}>
                        <View style={styles.historyHeader}>
                            <Text style={[styles.sectionTitle, { color: text, marginBottom: 0 }]}>出席履歴</Text>
                            <TouchableOpacity onPress={() => setShowAttendanceHistory(false)} style={styles.closeBtn}>
                        ...
                        <ScrollView style={styles.historyList}>
                            {(!course.attendanceHistory || course.attendanceHistory.length === 0) ? (
                                <View style={{ padding: 40, alignItems: 'center' }}>
                                    <Text style={{ color: subText }}>履歴がありません</Text>
                                </View>
                            ) : (
                                course.attendanceHistory.map((record) => {
                                    const date = new Date(record.date);
                                    const dateStr = `${date.getMonth() + 1}/${date.getDate()} (${['日', '月', '火', '水', '木', '金', '土'][date.getDay()]}) ${date.getHours()}:${date.getMinutes().toString().padStart(2, '0')}`;
                                    const statusColor = record.status === 'present' ? '#34D399' : record.status === 'late' ? '#FB923C' : record.status === 'absent' ? '#EF4444' : '#9CA3AF';
                                    const statusLabel = record.status === 'present' ? '出席' : record.status === 'late' ? '遅刻' : record.status === 'absent' ? '欠席' : '取消';

                                    return (
                                        <View key={record.id} style={[styles.historyItem, { borderBottomColor: itemBg }]}>
                                            <View style={{ flexDirection: 'row', alignItems: 'center', flex: 1 }}>
                                                <View style={[styles.historyDot, { backgroundColor: statusColor }]} />
                                                <View>
                                                    <Text style={[styles.historyStatus, { color: text }]}>{statusLabel}</Text>
                                                    <Text style={[styles.historyDate, { color: subText }]}>{dateStr}</Text>
                                                </View>
                                            </View>
                                            <TouchableOpacity
                                                onPress={() => {
                                                    Alert.alert(
                                                        ...
                                                        [
                                                            { text: t.cancel, style: "cancel" },
                                                            {
                                                                ...
                                                                style: "destructive",
                                                                onPress: () => {
                                                                    const updated = course.attendanceHistory!.filter(r => r.id !== record.id);
                                                                    onUpdate({ ...course, attendanceHistory: updated });
                                                                }
                                                            }
                                                        ]
                                                    );
