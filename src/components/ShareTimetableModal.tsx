...
import { Course, Day, DAYS, Settings } from '../lib/types';
    ...
    const handleShareFile = async () => {
        try {
            if (selectedCourses.length === 0) {
                Alert.alert(t.error, t.selectCoursesError);
                return;
            }
            ...
            await Sharing.shareAsync(fileUri, {
                ...
                dialogTitle: t.shareDataTitle
            });

        } catch (error) {
            console.error(error);
            Alert.alert(t.error, t.shareFileError);
        }
    };
        ...
        >
            <View style={[styles.container, { backgroundColor: bg }]}>
                ...
                <View style={styles.header}>
                    <Text style={[styles.title, { color: textPrimary }]}>{t.shareTimetable}</Text>
                    ...
                    {(['text', 'file', 'qr'] as const).map((tab) => (
                        ...
                        >
                            <Text style={[
                                ...
                                {tab === 'text' ? t.text : tab === 'file' ? t.file : t.qrCode}
                            </Text>
                        </TouchableOpacity>
                    ))}
                </View>
                        ...
                        <View style={styles.qrContainer}>
                            ...
                            {!qrStatus.valid ? (
                                <View style={[styles.qrWrapper, { alignItems: 'center', justifyContent: 'center', height: 240 }]}>
                                    ...
                                    <Text style={{ color: '#ef4444', marginTop: 12, fontWeight: 'bold' }}>{t.tooManyCourses}</Text>
                                    <Text style={{ color: textSecondary, marginTop: 8, fontSize: 12, textAlign: 'center' }}>
                                        {t.qrLimitInstruction(qrStatus.count)}
                                    </Text>
                                </View>
                                ...
                                <Text style={{ color: textSecondary }}>{t.selectCourses}</Text>
                            )}
                            <Text style={[styles.note, { color: textSecondary, marginTop: 20 }]}>
                                {t.selectedCount(selectedCourses.length)}
                            </Text>
                        </View>
                    ) : (
                        <>
                            <View style={styles.selectionHeader}>
                                <Text style={[styles.sectionTitle, { color: textPrimary }]}>
                                    {t.selectCoursesWithCount(selectedCourses.length, courses.length)}
                                </Text>
                                <View style={styles.selectionActions}>
                                    <TouchableOpacity onPress={handleSelectAll}>
                                        <Text style={styles.actionText}>{t.selectAll}</Text>
                                    </TouchableOpacity>
                                    <Text style={{ color: border }}>|</Text>
                                    <TouchableOpacity onPress={handleDeselectAll}>
                                        <Text style={styles.actionText}>{t.deselectAll}</Text>
                                    </TouchableOpacity>
                                </View>
                            </View>
                    ...
                    <View style={[styles.footer, { borderTopColor: border }]}>
                        ...
                        >
                            ...
                            <Text style={styles.shareButtonText}>
                                {activeTab === 'file' ? t.shareAsFile : t.shareAsText}
                            </Text>
                        </TouchableOpacity>
                    </View>
                )}
